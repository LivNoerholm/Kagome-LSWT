using StaticArrays, LsqFit, Random
...
################################################################################
mutable struct LangevinPlanck <: Sunny.AbstractIntegrator
    dt              :: Float64
    damping         :: Float64
    kT              :: Float64
...
function Sunny.step!(sys::Sunny.System{0}, integrator::LangevinPlanck)
...
function Sunny.step!(sys::Sunny.System{N}, integrator::LangevinPlanck) where N
    (Z′, ΔZ₁, ΔZ₂, ζ, HZ) = Sunny.get_coherent_buffers(sys, 5)
    Z = sys.coherents

    ζ = advance_and_retrieve_noise!(sys, integrator)

    # Euler prediction step
    Sunny.set_energy_grad_coherents!(HZ, Z, sys)
    Sunny.rhs_sun!(ΔZ₁, Z, ζ, HZ, integrator)
    @. Z′ = Sunny.normalize_ket(Z + ΔZ₁, sys.κs)

    # Correction step
    Sunny.set_energy_grad_coherents!(HZ, Z′, sys)
    Sunny.rhs_sun!(ΔZ₂, Z′, ζ, HZ, integrator)
    @. Z = Sunny.normalize_ket(Z + (ΔZ₁+ΔZ₂)/2, sys.κs)
